...
```{r}
#setwd("~/Uni programming/Experimental Methods 3/Assignment-1") ##Can't change wd after using version control
pacman::p_load(ggplot2,plyr,stringr,tidyverse,magrittr)
```
...
```{r}
# Try to visualize
ggplot(dword, aes(tokens_CHI, SUBJ)) +
  geom_line()

ggplot(dword, aes(VISIT, tokens_MOT)) +   
  geom_point()
## Visits are doubled

ggplot(dlength, aes(VISIT, MOT_MLU)) +   
  geom_point()
## Again - double visits
...
```{r}
# Rename demographic data subject and visit columns to match the ones for dlength and dword
...
```{r}
# Remove everything but the numbers of visits: the lines mean to only choose this and the d is digit so it leaves in only the numbers
...
```{r}
# Remove all dots from the subject names (because A.D. is the same as AD)
...
```{r}
# From each dataframe, select the variables you want to use, make dataframes
A = select(ddemo, SUBJ, VISIT, Ethnicity, Diagnosis, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw)
B = select(dlength, SUBJ, VISIT, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd)
C = select(dword, SUBJ, VISIT, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
```
...
```{r}
# Merge together in 2 steps because merge() can't handle otherwise
i = merge(A, B)
i = merge(i, C)
```
...
```{r}
# New dataframe of only visit 1's
newdata = i[i$VISIT == 1,]
# Rename the variables you'll merge with the big dataframe
...
# To factors
i$Diagnosis = as.factor(i$Diagnosis)
i$Gender = as.factor(i$Gender)

# Plot to check
counts = table(i$Gender, i$Diagnosis)
barplot(counts,
        col=c("green","blue"),
...
```{r}
# 1 kids who have a mean length of utterance (across all visits) of more than 2.7 morphemes
i$SUBJ = as.factor(i$SUBJ)

## Group by subject, find the mean for each kid across visits, filter by CHI_MLU > 2.7
temp = group_by(i, SUBJ) %>%
  summarise_each(funs(mean(., na.rm = TRUE)), CHI_MLU) %>%
  filter(CHI_MLU > 2.7)

# 2. have a mean length of utterance of less than 1.5 morphemes at the first visit
## Use AND operator for CHI_MLU lower than 1.5 and Visit equal to 1, select the 3 relevant columns
temp = filter(i, CHI_MLU < 1.5 & VISIT == 1) %>%
  select(SUBJ, VISIT, CHI_MLU)

# 3. have not completed all trials. Tip: Use pipes to solve this
## Find the sum of VISIT for all kids, select only the ones where sum of visit is less than 21
temp = i %>%
  group_by(SUBJ) %>%
...
```{r}
# 1. Sort kids to find the kid who produced the most words on the 6th visit
## Arranges in a descending order by tokens, filters by taking only the rows that show visit 6, selects from those only 3 columns, [1,] displays only 1st row
temp = select(filter((arrange(i, -tokens_CHI)), VISIT == 6), SUBJ, VISIT, tokens_CHI)[1,]

# 2. Sort kids to find the kid who produced the least amount of words on the 1st visit.
## arrange() - when there is a minus sign before the thing then it decreases, when not then increases
temp = select(filter((arrange(i, tokens_CHI)), VISIT == 1), SUBJ, VISIT, tokens_CHI)[1,]
```
...
```{r}
# 1. Make a subset of the data including only kids with ASD, mlu and word tokens
temp = filter(select(i, SUBJ, Diagnosis, CHI_MLU, tokens_CHI), Diagnosis == "ASD")

# 2. What happens if you include the name of a variable multiple times in a select() call?
temp = select(i, SUBJ, SUBJ, SUBJ)
