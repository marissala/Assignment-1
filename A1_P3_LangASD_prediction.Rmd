...
folds = createFolds(unique(d_train$SUBJ), 4)
...
for(i in folds) {
  ...
  dtestt = subset(d_train, SUBJ %in% i)
  #train model
  modelt = lmer(CHI_MLU ~ VISIT*Diagnosis + uADOS*verbalIQ + uADOS*Diagnosis + tokens_CHI + MOT_MLU + (1+VISIT|SUBJ), dtraint)
  #test the model, traindata
...
verbIQ = mean(subset(d_train, Diagnosis == "TD")$verbalIQ)
CHI = mean(subset(d_train, Diagnosis == "TD")$CHI_MLU)

#average kid dataset, avg VerbIQ and avg MLU
kiddo = data.frame(SUBJ = rep(111,6), VISIT = c(1:6), Diagnosis = rep("TD", 6), verbalIQ = rep(verbIQ, 6))

# predict MLU
predict(modelt, kiddo, allow.new.levels=T)
